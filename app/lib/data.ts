...
export async function fetchPaidInvoiceTotalAmount() {
  try {
    const data = await sql<{ total: string }>`
    ...
    console.error("Database Error:", error);
    throw new Error("Failed to fetch total invoice amount.");
  }
}

export async function fetchPendingInvoiceTotalAmount() {
  try {
    const data = await sql<{ total: string }>`
